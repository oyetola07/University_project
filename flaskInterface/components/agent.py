...
import os
sys.path.insert(0, '')
from . import outputkggeneration as okgg
from . import searchandinvoke as sandi
from . import inputkggeneration as ikgg
#perfromsistant the best_model in the disc but described as triple knowledge 
...
def OperatingTask (task_name,input_file_path,desir_output_type,task_domain,namespace,purpose):
    print(servicelist)
    g = Graph()
    g.parse(component+"contextkg.n3")
    openflag = 0
    purpose_flag = 1
    if len(task_name) > 0 and len(desir_output_type)> 0:
        ...
        g.serialize(destination=component+"contextkg.n3"+".n3")
        g.close()
        ctx=''
        if openflag ==1:
...
def reset_knowledge():
    open(component+'contextkg.n3', 'w').close()
    open(component+'policy.n3', 'w').close()
    open(component+'commonkg.n3', 'w').close()
    open(component+'workflows.n3', 'w').close()
    mydir = component+'models/'
...
def commonKGlearning(datasetout,task_id,outputv,ns):
    ...
    #print(kg)
    kg.serialize(destination=component+'taskinput'+".n3")
    kg.close()
...
def task_output_workflowKG (listOfms,task_id,ns):
    ...
    if len(listOfms)>0:
        kg = Graph()
        kg.parse(component+"workflows.n3")
...
def savemodel(task_id,model,ns):
    ...
    #print(kg)
    kg.serialize(destination=component+'solution'+".n3")
    kg.close()
